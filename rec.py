...
def op_cond(args, env):
    while args is not EL:
        pc, args = unpack(car(args), 2), cdr(args)
        predicate, consequent = pc
...
def leval(x, env=GLOBALS):
    ...
    if not getattr(proc, "special", False):  ## specials get unevaluated args
        lb = ListBuilder()  ## regular lambdas get evaluated args
        while args is not EL:
            arg, args = leval(car(args), env), cdr(args)
            lb.append(arg)
        args = lb.get()
    ret = proc(args, env)
